...
interface DepartmentContractChartProps {
  data: DepartmentData[];
}

export const DepartmentContractChart = ({ data }: DepartmentContractChartProps) => {
  // データを実績でソート
  ...
  return (
    <Card className="border border-border">
      <CardHeader>
        <CardTitle className="text-lg font-semibold">統括部別新規契約数及び目標達成率</CardTitle>
      </CardHeader>
      <CardContent>
          ...
          >
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => {
                if (name === "実績") return [`${value.toLocaleString()}件`, name];
                if (name === "計画") return [`${value.toLocaleString()}件`, name];
                return [value, name];
              }}
            />
            <Legend 
            ...
            />
            {/* 実績バー */}
            <Bar dataKey="actual" fill="hsl(var(--chart-2))" name="実績" radius={[0, 4, 4, 0]} barSize={20} />
            {/* 計画値をラインマークで表示 */}
            <Scatter 
              dataKey="plan" 
