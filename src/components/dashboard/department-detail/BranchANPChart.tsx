...
interface BranchANPChartProps {
  data: BranchData[];
}

export const BranchANPChart = ({ data }: BranchANPChartProps) => {
  // データを実績でソート
  ...
  return (
    <Card className="border border-border">
      <CardHeader>
        <CardTitle className="text-lg font-semibold">支社別新規ANP及び目標達成率</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={Math.max(400, data.length * 35)}>
          ...
          >
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-popover border border-border rounded-lg p-2 shadow-lg">
                      ...
                      <p className="text-sm">実績: ¥{(payload[0].payload.actual / 1000000).toFixed(0)}M</p>
                      <p className="text-sm">計画: ¥{(payload[0].payload.plan / 1000000).toFixed(0)}M</p>
            ...
            />
            {/* 実績バー */}
            <Bar dataKey="actual" fill="hsl(var(--primary))" name="実績" radius={[0, 4, 4, 0]} barSize={18} />
            {/* 計画値をラインマークで表示 */}
            <Scatter 
              dataKey="plan" 
