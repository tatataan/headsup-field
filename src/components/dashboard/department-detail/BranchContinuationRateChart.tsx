...
interface BranchContinuationRateChartProps {
  data: BranchData[];
}

export const BranchContinuationRateChart = ({ data }: BranchContinuationRateChartProps) => {
  // データを実績でソート
  ...
  return (
    <Card className="border border-border">
      <CardHeader>
        <CardTitle className="text-lg font-semibold">支社別継続率及び目標達成率</CardTitle>
      </CardHeader>
      <CardContent>
        <ResponsiveContainer width="100%" height={Math.max(400, data.length * 35)}>
          ...
          >
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-popover border border-border rounded-lg p-2 shadow-lg">
                      ...
                      <p className="text-sm">実績: {payload[0].payload.actual.toFixed(1)}%</p>
                      <p className="text-sm">計画: {payload[0].payload.plan.toFixed(1)}%</p>
            ...
            />
            {/* 実績バー */}
            <Bar dataKey="actual" fill="hsl(var(--chart-4))" name="実績" radius={[0, 4, 4, 0]} barSize={18} />
            {/* 計画値をラインマークで表示 */}
            <Scatter 
              dataKey="plan" 
